...
var instrucciones = ["Hacer una cosa","Hacer otra cosa","Y otra cosa"];
...
function ultimaDireccion(udireccion){
movimientos.push(udireccion);
actualizarUltimoMovimiento(udireccion)
}
...
for(i=0;i<grilla.length;i++){
  for(j=0;j<grilla[i].length;j++){
   if(grilla[i][j]==ganador[i][j]){coincidencias++}
}}
if (coincidencias==9){return  true} else {return false}
}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  var anteriorA = grilla[filaPos1][columnaPos1];
  var anteriorB = grilla[filaPos2][columnaPos2];

  grilla[filaPos1][columnaPos1]=anteriorB;
  grilla[filaPos2][columnaPos2]=anteriorA;
  // for(i=0;i<grilla.length;i++){
  //   for(j=0;j<grilla[i].length;j++){

  //     if(i==filaPos1&&j==columnaPos1){
  //     grilla[i][j]=grilla[filaPos2][columnaPos2]
  //     }
  //     if(i==filaPos2&&j==columnaPos2){
  //       grilla[i][j]=grilla[filaPos1][columnaPos1];
  //   }}}
}
...
function posicionValida(fila, columna) {
    if(fila>2||fila<0||columna>2||columna<0){return false}else{return true}
}
...
function moverEnDireccion(direccion) {
  ...
  las funciones posicionValida, intercambiarPosicionesGrilla y actualizarPosicionVacia */
    ...
    if (posicionValida(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia)) {
      ...
      // if(chequearSiGano(grilla)==true){mostrarCartelGanador()}
}
